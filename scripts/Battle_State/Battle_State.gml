...
function Battle_SetState(_state) {
    ...
    switch (_state) {
        ...
        case BATTLE_STATE.TURN_PREPARATION:
			...
			{
				x = global.main_battle_board.x + Battle_GetTurnInfo(BATTLE_TURN.SOUL_X, 0);
				y = global.main_battle_board.y + Battle_GetTurnInfo(BATTLE_TURN.SOUL_Y, 0);
			}
...
///@param {String}		type				The type of transformation; "move" for position, "rot" for rotation, or any other value for size.
...
function Battle_BoardTransform(_type, _old_values, _target_values, _ease, _mode, _speed, _duration)
{
	...
	if (_type == "move") {
		...
		_properties = ["image_angle>"];
	} else {
		_properties = ["up>", "down>", "left>", "right>"];
	}
    var _durations = [];
...
function Battle_BoardTransforming(_state_prefix)
{
	...
	{
		case "set":
		    var _move_old     = [global.main_battle_board.x, global.main_battle_board.y];
		    var _move_target  = [
                                Battle_GetTurnInfo(BATTLE_TURN.BOARD_X, BATTLE_BOARD.X),
                                Battle_GetTurnInfo(BATTLE_TURN.BOARD_Y, BATTLE_BOARD.Y)
                              ];
		    var _size_old     = [global.main_battle_board.up, global.main_battle_board.down, global.main_battle_board.left, global.main_battle_board.right];
		    var _size_target  = [
			...
			var _rot_old     = [global.main_battle_board.image_angle mod 360];
		    var _rot_target  = [Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT, _rot_old[0] <= 180 ? 0 : 360)];
		    var _rot_ease     = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_EASE, EaseInOutSine);
		    var _rot_mode     = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_MODE, BATTLE_TURN_BOARD_TRANSFORM_MODE.SPEED);
		    var _rot_speed    = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_SPEED, 5);
		    var _rot_duration = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_DURATION, 30);
		...
		case "reset":
		    ...
		    var _rot_target  = [Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT, _rot_old[0] <= 180 ? 0 : 360)];
		    var _rot_ease     = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_EASE, EaseInOutSine);
		    var _rot_mode     = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_MODE, BATTLE_TURN_BOARD_TRANSFORM_MODE.DURATION);
		    var _rot_speed    = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_SPEED, 5);
		    var _rot_duration = Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_ROT_DURATION, 30);
